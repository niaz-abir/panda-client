...
const AllJersey = () => {
  ...
  const handleDelete = async (id: string) => {
    try {
      await axios.delete(
        `https://panda-server-eight.vercel.app/api/v1/delete-jersey/${id}`
      );
          ...
          data?.map((jerseyCrd: TJersey) => (
            ...
            >
              ...
              <div className="p-4">
                ...
                <h3 className="mb-2 font-bold">
                  Category:{" "}
                  <span className="font-semibold ">{jerseyCrd.category}</span>{" "}
                </h3>
                <h3 className="mb-2 font-bold">
                  Quality:{" "}
                  <span className="font-semibold ">{jerseyCrd?.quality}</span>{" "}
                </h3>
              </div>
            </div>
          ))
